...
def collect_run(args, source_args, backend, purpose, repeat, artifacts, compact, environment):
    ...
    command = [sys.executable, str(Path(__file__).resolve())] + source_args + [
        "--h5", str(args.h5), "--worker", backend, "--compact-data", str(compact), "--purpose", purpose,
...
def summarize(report, args):
    ...
    report["status"] = "complete" if (
        ...
        and report["matched_device_identity"]
    ) else "failed"
...
def main(argv=None):
    ...
    report = dict(
        schema_version=2, status="running", h5=str(args.h5), artifacts_dir=str(artifacts),
        configuration={key: str(value) if isinstance(value, Path) else value for key, value in vars(args).items()
                       if key not in {"worker", "compact_data", "worker_report", "worker_factor", "purpose"}},
        ...
        interpretation=[
            ...
            "Conversion is paid once separately; per-worker process time includes imports, device initialization, load, run, save and shutdown.",
            "No result plots are generated by the benchmark; saved outputs are comparable final factors and JSON/logs.",
    ...
    try:
        ...
        report.update(compact_save_seconds=perf_counter() - started, n_qubits=data.n_qubits,
                      observations_preserved=True, verification="full", source_metadata=data.metadata,
                      source_fingerprint=fingerprint, memory_estimates=memory_estimates(data, args),
                      converter_parent_peak_rss_bytes=process_peak_rss_bytes())
        write_json(args.save, report)
        schedule = [
        ...
        if source_fingerprint(args.h5) != fingerprint:
            raise RuntimeError("HDF5 source changed during the benchmark.")
