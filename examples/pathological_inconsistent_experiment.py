"""
Pathological inconsistent indicator experiment.

This example shows that the best-approximation value residual need not control
distance to the closest-point set at a quadratic rate.

    C = {(x1, x2): -1 <= x1 <= 1, x1^4 <= x2 <= 1},
    T(x1, x2) = x2,
    D = {-1}.

Then T(C) = [0, 1] is disjoint from D, delta = 1, and
C^\dagger = {(0, 0)}.  Along the lower boundary x2 = x1^4,
h(x)-h^* is proportional to x1^4 while dist_{C^\dagger}(x) is proportional
to |x1|.
"""
...
_cache_root = Path(tempfile.gettempdir()) / "pathological_inconsistent_matplotlib"
...
DEFAULT_OUTFILE = THIS_DIR / "paper_fig_pathological_inconsistent.pdf"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run the pathological inconsistent experiment and save a paper figure."
    )
...
def linf_cusp_lmo(gradient):
    """
    Linear minimization oracle over C = {x1^4 <= x2 <= 1, |x1| <= 1}.
    """
...
def run_trace(n_steps, beta0, step_shift, center=None, x_initial=None):
    ...
    for k in range(n_steps):
        ...
        direction = linf_cusp_lmo(grad_f + moreau_grad)
...
def semilogy_with_floor(ax, x_values, y_values, **kwargs):
    ax.semilogy(x_values, np.maximum(y_values, 1e-16), **kwargs)
...
def draw_geometry(ax, trace):
    ...
    ax.set_title("Flat Growth", fontsize=titlefontsize)
...
def draw_growth(ax, trace):
    x_grid = np.geomspace(1e-4, 1.0, 400)
    ...
    ref = np.geomspace(1e-3, 1.0, 100)
    ax.loglog(ref, ref**2, "--", color="0.45", linewidth=1.6, label=r"$r^2$")
    ax.loglog(ref, ref**4, ":", color="0.2", linewidth=2.0, label=r"$r^4$")

    ax.set_title("Growth Near $\\mathcal{C}^\\dagger$", fontsize=titlefontsize)
...
def save_figure(trace, beta0, step_shift, outfile, dpi):
    ...
    fig.suptitle(
        rf"Pathological Inconsistent Geometry; "
...
def print_diagnostics(trace, n_steps, beta0, step_shift, outfile):
    ...
    print("Pathological inconsistent experiment")
    print(r"  C = {(x_1, x_2): -1 <= x_1 <= 1, x_1^4 <= x_2 <= 1}")
    ...
    if trace.h_gap[-1] > 0.0:
        print(
            "  "
            f"dist_Cdag/(h-h*)^(1/4)="
            f"{trace.cdagger_distance[-1] / trace.h_gap[-1] ** 0.25:.6f}"
        )
