...
chunk_steps = 10
if case.get("profile_chunks"):
    subprocess.run([sys.executable, str(root / "profile_continuation_chunks.py"),
        "--provenance", case["profile_provenance"], "--source", str(source),
        "--output", str(output / "chunk_profile")], check=True)
    chunk_report = json.loads((output / "chunk_profile" / "report.json").read_text())
    chunk_steps = chunk_report["selected_chunk_steps"]
    assert chunk_steps in (1, 10)
...
common = [sys.executable, "-m", "paper.experiments.quantum_process_tomography_structured_jax",
    ...
    "--batch-size", str(case["batch_size"]), "--chunk-steps", str(chunk_steps), "--prefetch",
