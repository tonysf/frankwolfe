...
def main():
    ...
    for name in args.cases:
        ...
        job_id = '1931802_' + str(case['array_task'])
        ...
        assert provenance['source_sha256'] == selection['source_sha256']
        ...
        assert '--resume' not in command and '--resume-from' not in command and '--fidelity-target' not in command
        expected_flags = {
            ...
            'rho-scale': case['rho_scale'], 'rho-offset': case['rho_offset'], 'steps': case['steps']}
        ...
        previous_requested = read(directory / 'requested_baseline_trace.json')
        assert previous_requested['metadata']['initial_factor_sha256'] == case['initial_factor_sha256']
        assert previous_requested['metadata']['batch_symbols_sha256'] == metadata['batch_symbols_sha256']
        requested_endpoint = previous_requested['trajectory'][-1]
        assert requested_endpoint['step'] == case['steps']
        ...
        record = dict(case=name, status='verified_and_backed_up', checked_at_utc=now,
                      ...
                      requested_exponent_baseline=requested_endpoint,
