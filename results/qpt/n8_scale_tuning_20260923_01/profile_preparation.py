...
env=dict(os.environ,QPT_TEST_DEVICE='gpu')
with (OUT/'tests.log').open('x') as log:
    subprocess.run([sys.executable,'-m','pytest','-q','tests/test_qpt_restart.py',
                    'tests/test_qpt_on_demand.py','tests/test_qpt_product_state.py'],
                   cwd=PLAN['optimized_source'],env=env,stdout=log,stderr=subprocess.STDOUT,check=True)
...
def run(source,name,steps=1000,resume=None):
    ...
    assert meta['noise_std']==.05 and meta['initial_factor_sha256']==case['initial_factor_sha256'] if not resume else meta['start_step']==100
    return arrays,meta
...
selection=dict(status='validated',selected=selected,
    ...
    gpu_kind=device.device_kind,checks=checks+['pytest','three_repeat_full_factor_momentum_scalar_bitwise_equivalence','original_to_fast_restart_bitwise_equivalence'],
