"""Run the authorized exponent-only comparison inside a Slurm allocation."""
import argparse
import datetime
import hashlib
import json
...
def check_output(path,case,steps):
    ...
    expected={'step_sizes':np.minimum(1,case['step_scale']/(k+case['step_offset'])**.75),
              ...
              'momentum_weights':np.minimum(1,2/(k+4)**.5)}
    with np.load(path,allow_pickle=False) as z:
...
def run(case):
    ...
    try:
        ...
        assert hashes==PLAN['source_sha256'],'Validated frozen source changed'
        ...
        common=[sys.executable,'-m','paper.experiments.quantum_process_tomography_structured_jax',
            ...
            '--rho-scale','2','--rho-offset','4','--rho-exponent','0.5',
